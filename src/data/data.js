import paleta from '../assets/paleta.png'
import landding from '../assets/landing.png'
import triqui from '../assets/triqui.png'

export const dataPortfolio = [
    { 
      id: 1,
      title: "Landing Pages",
      description: "Este proyecto freelance es un landding pages para una tienda de comida rápida  ",
      image:landding ,
      url: "https://landing-comida.netlify.app",
      repo: "",
    },

    { 
      id: 2,
      title: "Paleta de colores",
      description: "Este proyecto es un generador de paleta de colores",
      image: paleta,
      url: "https://generadordedcolores.netlify.app",
      repo: "",
    },

    { 
      id: 3,
      title: "Triqui",
      description: "Este proyecto se basa en un tradicional juego movil de triqui con temática de spiderman ",
      image: triqui,
      repo: "https://github.com/carol-puentes/TRIQUI",
    },
  ];
  
